...
class App(QMainWindow):
    ...
    def _show_app_update_available(self, result: UpdateCheckResult) -> None:
        box, open_btn = self._create_app_update_box(result)
        box.exec()
        if box.clickedButton() is open_btn:
    ...
    def _create_app_update_box(
        ...
        """新版検出ダイアログを組み立てる（表示内容をテスト可能にする分離）。

        通知＋「リリースページを開く」導線のみを提供する（実体の自動更新は
        #276 で撤去。spec: docs/spec/features/app-update.md）。
        """
        box = QMessageBox(self)
        box.setIcon(QMessageBox.Icon.Information)
        box.setWindowTitle(t("menu_about"))

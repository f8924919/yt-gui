...
C-1 残の完了をもって、ユーザー判断により **D 群（investigate 役割定義の文言・git-workflow の番号体系二重化）は見送り**（実害なし・任意のため）で確定し、本調査タスクを完了とする。B-2 は保留のまま（運用実績を踏まえた棚卸しタイミングで必要になれば再起票する）。残る軽微な確認事項は「次回セッションでの hook 実効確認」のみ（hooks はセッション開始時読込のため本セッションでは検証不能。main 上で `git commit` がブロックされることを確認する）。
...
- 現状: pytest は Ubuntu ランナー単体（`test.yml:20`）。単一 OS は `docs/testing/index.md:35` に明記された意図的選択だが、アプリは Windows / macOS 向け。実際に Windows 固有バグの実績あり（#201: `SO_REUSEADDR` の Windows 仕様差でポートフォールバック不発。Windows 実機でのみ再現）。（2026-07-11 C-2 着手 #227 により解消。行番号参照は調査当時のもの）
...
- [x] C-3 の対応（2026-07-11 完了。#224 = PR #225。omit 解除・TOTAL 約 81%）
- [x] C-2 の対応（2026-07-11 完了。#227 = PR #228。test-windows ジョブ追加・必須チェック化・fixture の OS 非依存化）
- [x] C-4 の対応（2026-07-11 完了。#229 = PR #230。CodeQL 導入・初回スキャン alert 0 件）
- [x] C-1 残の対応（2026-07-11 完了。#232 = PR #233。hook + enforce_admins 有効化。pre-commit は見送り）
- [x] B-2 の扱いの確定（2026-07-11、ユーザー判断で保留。現状問題なし・運用実績を踏まえた棚卸しタイミングで再検討）
- [x] D 群の扱いの確定（2026-07-11、ユーザー判断で見送り。実害なし・任意のため）
- [x] タスク完了（2026-07-11。次回セッションでの hook 実効確認のみ軽微な残確認として引き継ぎ）

...
- PR #6 を `main` へマージ済み。マージ後の初回実行で `v0.1.0` タグが作成され、リリースワークフローが起動。
- **push ブロッカーの真因（重要・環境固有）**: トークンスコープ不足ではなく、**サンドボックスの egress プロキシ（`gateway.docker.internal:3128`、MITM）が github.com 宛の `Authorization` をガバナンス管理の固定トークンに差し替えていた**こと。当初その固定トークンに `workflow` スコープが無く `.github/workflows/` の push が拒否されていた。検証法: 無効トークン／認証ヘッダ無しでも `api.github.com` が 200 を返し同一スコープになる。ローカル `GH_TOKEN` の再発行では解決せず、プロキシ側トークンへ `workflow` を付与して解消した。

...
| [#240](https://github.com/f8924919/yt-gui/issues/240)（クロスリポジトリ） | 記述なし（[240-hook-cross-repo.md](archive/240-hook-cross-repo.md) は「subprocess 起動方式を踏襲」） | **既存方式の踏襲のみ**で、選んだ理由は書かれていない |
...
### 当初この節に書いていた誤り（`evaluator` が検出・訂正ログ 1 件目）
...
| `docs/task/archive/240-hook-cross-repo.md`「`tests/test_block_main_commit.py` 新設（**importlib 読込**・一時 git リポジトリで検証）」 | **`#337` への参照ではなく、§1 で訂正したのと同じ誤りの独立した主張**（`evaluator` が検出。当初の洗い出しは語 `importlib で読み込み` で引いたので `importlib 読込` に当たらなかった）。**直さない判断**: archive は**その時点の記録**で、書き直すと「当時そう理解していた」という事実まで消える（[docs-guide.md](../docs-guide.md) §4.2）。**誤りの訂正の正本は `policy.md` §1 と本メモ**で、archive からそこへ辿れる（同メモの表がこの行を名指ししている）。なお `docs/task/archive/285-template-backport.md` の「subprocess 実行だけでは deny 経路を再現できない」は新しい記述と整合する |
...
**教訓: 「数を書いていないか」を引くときは、数の単位を列挙で当てにいくと必ず漏れる。** 単位を広く採るか、`[0-9]` と漢数字の出現そのものを見て 1 件ずつ読む方が確実。最終の確認は `[0-9]+ *(本|件|個|つ|分岐|箇所|行|回|通り|種|ケース)|全 [0-9]+` で 0 件。
...
| 区分 | 指摘 | 閉じ方の種別 | 証跡 |
|---|---|---|---|
| `[欠陥]` | `policy.md` §1 の経緯「`settings.json` と同じ起動形式を pytest で再現し、セッション内での発火確認の代替にした」が偽で、同じ節の 9 行下の「登録形式はいずれのテストも検証していない」と矛盾 | **条件改訂 + 記述の書き換え**（ユーザー判断・案 c: 経緯を落として事実だけにする）。Issue #337 の C3 の指示文も訂正した | `docs/testing/policy.md` §1 の `block_main_commit.py` の行（「**この方式を選んだ理由は一次資料で裏取りできていない**ので、ここには書かない」）・本メモ「C1 の記録」の一次資料 3 点の表・訂正ログ 1 件目 |
...
|---|---|---|---|
| 2026-09-22 | `policy.md` §1 に「`settings.json` と同じ起動形式を pytest で再現し、セッション内での発火確認の代替にした経緯」と書いた | **pytest は登録形式を再現していない**（初版から `sys.executable` 直叩き）。PR #233 の「疎通確認」は**手動 smoke** の話。**同じ節の 9 行下の記述と正面から矛盾していた** | **書いた節を通しで読み直すこと**（矛盾する 2 文が 9 行差で同居していた）。`investigate` が引いた文面を、**pytest の話か手動 smoke の話かを確かめずに転記した**のが原因 — 引用は出所の文脈まで当たる（§8.3 C2）。`evaluator` が一次資料を読んで検出 |

...
- Issue 本文・コメントは日本語で記述する（[CLAUDE.md 言語ルール](../CLAUDE.md#言語ルール)に従う）。
...
3. 変更を実装し、対応する `docs/` を併せて更新（[docs-guide.md](docs-guide.md) §4）。
4. lint / 型チェック / テストを通す（[CLAUDE.md](../CLAUDE.md) 参照）。
5. `gh` で PR を作成（ベース `main`、本文は日本語、関連 Issue を `Closes #<issue>` で紐付け）。
6. **ユーザーの承認後**にマージし、マージ済みブランチを削除。

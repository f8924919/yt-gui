...
機械的・探索的な作業は読み取り/検証用のサブエージェント（いずれも Sonnet）へ委譲し、主エージェントの文脈を温存する。定義は `.claude/agents/` 配下。**設計・仕様の判断、テスト内容の決定、設計外の問題への対応は委譲せず、主エージェントとユーザーが行う**（§5.1）。

| エージェント | 委譲する作業 | 対応するフロー | 主エージェントが受け取るもの |
|---|---|---|---|
| [`investigate`](../.claude/agents/investigate.md) | docs 先・コード裏取りの調査 | step 3 | 結論・関連 `path:line`・裏取りメモ |
| [`verify`](../.claude/agents/verify.md) | lint / フォーマット / 型 / テストを green にする | step 7 | 検証結果・修正点・要判断項目 |
| [`docs-check`](../.claude/agents/docs-check.md) | docs 整合性の点検と機械的修正 | step 7 | 点検結果・修正点・要対応項目 |

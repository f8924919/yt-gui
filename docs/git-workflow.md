...
| **証跡・文言** | **docs・タスクメモ・コメントの編集**（既存のテストの再実行と貼り直しを含む）だけで閉じる指摘。主張が実測を上回る**記述**、記述の網羅性、証跡の**鮮度**（採取後にコードが変わった）、行番号・名前の腐り、docs・報告文中のラベルの分かりにくさ（**hook の注入文・テストの失敗メッセージなど、ゲートが出力する文言は「ゲートの出力」側 = 欠陥**） | **ブロックしない**。同じ PR で直すか、follow-up Issue に落として PR を進める。**どちらにしたかを報告に書く**（Issue を作らず握りつぶすのは不可） |
...
5. **「要判断」（spec とコードの食い違いなど、評価者が決められない事項）は区分の外に置く。** 要判断が残る巡は、ユーザーの決定を待って止まる（**【通知】** §5.8）。決定を受けて直したものは、閉じ方に応じて 3・4 に従う。要判断を残したまま再評価を回すと、同じ指摘が巡ごとに再生産される。
...
| [`verify-gate`](../.claude/skills/verify-gate/SKILL.md) | 手順 2（`verify` に設計判断が要る失敗が残ったとき）・手順 4（モード行が見つからないとき）・手順 5（`verify` / `docs-check` の FAIL、`evaluator` に `[欠陥]` または要判断が残るとき） |

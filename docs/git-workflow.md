...
>
> **証跡の「無い」と「古い」は、タスク単位・検査単位で分ける**（変異 1 件ごとではない）。**無い** = そのタスクで変異 → red を一度も示していない → 欠陥（閉じるには変異を入れて確かめる必要がある）。**古い** = 示したあとにコードが動き、貼ってある記録が最終状態と違う → 証跡・文言（**同じ変異の再実行と貼り直し**で閉じる）。
...
| [`verify-gate`](../.claude/skills/verify-gate/SKILL.md) | 手順 2（`verify` に設計判断が要る失敗が残ったとき）・手順 4（モード行が見つからないとき）・手順 5（`verify` / `docs-check` の FAIL、`evaluator` に `[欠陥]` が残るとき） |

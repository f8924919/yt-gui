...
- **`_bind_translation(key, setter, getter, *, transform=None)`**: バインディングを `self._translation_bindings: list[_TranslationBinding]` に登録し、**その場で初期テキストも設定する**汎用ヘルパ。QMenu の `setTitle`/`title`、ツリーヘッダー列（`headerItem().setText(col, …)`）などはクロージャで渡す。

...
動画全体ではなく**指定した時間範囲だけ**を切り出してダウンロードする機能です（yt-dlp の `--download-sections` 相当）。長尺動画から必要な部分だけを取得したいときに使います。**既定は無効**です。

### 指定方法

- メインウィンドウの区間指定 UI（[メインウィンドウ — 区間指定フィールド](../screens/main-window.md#区間指定フィールドダウンロード範囲)）で、有効化チェックをオンにし、**開始・終了**を入力します。
- 時刻は `HH:MM:SS` / `MM:SS` / 秒数（`90`）のいずれの表記でも受け付けます（`yt_dlp.utils.parse_duration` で解釈）。
...
- 既定（OFF）: **キーフレーム境界**でカットします。再エンコードしないため高速・無劣化ですが、指定時刻ぴったりにはならず最寄りのキーフレームにずれます。
- ON（`force_keyframes_at_cuts`）: カット境界を**再エンコード**し、指定時刻に正確に合わせます。処理時間が増えます。

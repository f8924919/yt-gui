...
動画の**指定した時間範囲だけ**を出力として得る機能です。長尺動画から必要な部分だけを切り出したいときに使います。**既定は無効**です。
...
- メインウィンドウの区間指定 UI（[メインウィンドウ — 区間指定フィールド](../screens/main-window.md#区間指定フィールドダウンロード範囲)）で、有効化チェックをオンにし、**開始・終了**を入力します。
...
1 つの時間範囲を異なる複数動画へ一律適用しても無意味なため、**プレイリストには適用できません**。URL 入力時点では単一動画かプレイリストかを判定できないため、タイトル取得後にプレイリストと判明し、かつ区間が指定されている場合は警告を表示して追加を中断します（[メインウィンドウ — 区間指定フィールド](../screens/main-window.md#区間指定フィールドダウンロード範囲)）。同様に、複数アイテムをまとめて編集している間は区間 UI を無効化します。

### 正確なカット（再エンコード）

切り出し時の ffmpeg の動作を切り替えます。

...
- キューのロックが保護するのは**キュー項目の状態遷移のみ**で、ダウンロード実行（出力ファイルパスの解決を含む）はロック外で並列に走る。同一ファイル名へ解決される項目同士の衝突は、キューのロックとは**別の排他機構**（`Downloader` の出力パス予約。[ダウンロード動作 — 並列ダウンロード時の衝突回避](download-behavior.md#並列ダウンロード時の衝突回避パス予約)）が防ぐ（#249）

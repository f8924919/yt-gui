...
| 操作 | 動作 |
|---|---|
| 「一時停止」ボタン | `_paused = True` フラグを立て、**進行中のダウンロードを即座に中断**する。中断されたアイテムは `waiting` に戻り（`error` にはしない）、`_worker` スレッドが停止する |
| 「ダウンロード開始」ボタン（再開） | `_paused = False` にリセットして `_worker` スレッドを再起動。`waiting` に戻ったアイテムを含め先頭から再走行する |

### 進行中ダウンロードの中断

「一時停止」を押すと、現在進行中のダウンロードを途中で中断できる。

- 中断は yt-dlp の `progress_hook` 内で `DownloadCancelled` を投げる**協調的**な仕組み。フラグメント DL 中は概ね即時、メタデータ抽出中やポストプロセス中は当該フェーズ完了後に効く（ベストエフォート）。

...
| C6 | **サブエージェントの報告にある数・件数・回数**を、受け入れ条件・Issue 本文・タスクメモへ引用する箇所 | **一次資料（index・一覧・コマンドの出力などの機械出力）を自分で数えたコマンドとその出力**（回数・経緯は数えるのではなく、示された `path:line` を開いて確かめた旨）。報告者の視野がそのまま母集団になる事故が繰り返し起きた — 「兄弟 5 本」を調査役が**その 5 本だけ見て**「前提はずれていない」と返し、一次資料を数えたら 16 本だった |

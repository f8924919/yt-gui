...
- **`yt_gui/locales/ja.py`** / **`yt_gui/locales/en.py`** — 各言語の文字列辞書（`STRINGS: dict[str, str]`）。新言語追加時はこのパターンで `xx.py` を追加し、`i18n.py` の `_LANGUAGES` に登録する。`fmt_720p` / `fmt_mp3` の値はテンプレート文字列（`{resolution}` / `{bitrate}` プレースホルダーを含む）になっており、`App._build_format_display()` が設定値を埋めて表示名を生成する。FLAC選択時は `fmt_flac` キーを使用する。コンテキストメニュー項目（`ctx_copy_url`・`ctx_edit_format` など）もここに定義する。
- **`yt_gui/formats.py`** — `FORMAT_SPECS` 定数（内部キー → `(yt-dlpフォーマット文字列, 音声のみフラグ)` のタプル）と `FORMAT_KEYS`（表示順リスト）。また `VIDEO_RESOLUTIONS`（`"480"` 〜 `"2160"` のタプル）・`MP3_BITRATES`（`"128"` 〜 `"320"` のタプル）・`AUDIO_FORMATS`（`("mp3", "flac")` のタプル）を定義する。
- **`yt_gui/utils.py`** — `strip_ansi(text: str) -> str`。ANSIエスケープコードを除去する共通ユーティリティ。（変更なし）
- **`yt_gui/downloader.py`** — `Downloader` クラス。yt-dlpのラッパー。`__init__` で `video_resolution`・`mp3_bitrate`・`log_callback`・`status_callback` を受け取る。`fetch_title_or_entries()` でURL種別判別（戻り値に `thumbnail_url: str | None` を含む）、`fetch_formats()` でフォーマット一覧取得、`download_video()` でダウンロード実行。`download_video` は `audio_codec: str = "mp3"` パラメータを受け取り、FFmpegExtractAudio の `preferredcodec` に使用する。ビットレート指定・サムネイル埋め込みは MP3 時のみ有効。出力拡張子（`.mp3` / `.flac`）も `audio_codec` から自動決定する。`_ffprobe_path` も `_ffmpeg_path` と同様に `bin/ffmpeg/ffprobe[.exe]` として解決し、yt-dlpに渡す。Cookies はファイルパス・ブラウザ名の両方に対応。
- **`yt_gui/settings.py`** — `Settings` dataclassと `SettingsManager` クラス。設定をJSONファイルに読み書きする。フィールド: `cookies_path` / `cookies_browser` / `download_path` / `language` / `video_resolution` / `mp3_bitrate` / `audio_format`（`"mp3"` または `"flac"`、デフォルト `"mp3"`）。保存先はOS標準のconfigディレクトリ（Windows: `%APPDATA%/yt-gui/`、macOS: `~/Library/Application Support/yt-gui/`）。
- **`yt_gui/settings_dialog.py`** — `SettingsDialog(QDialog)` クラス。モーダルの設定画面。`QTabWidget` によるタブ構成（「一般」・「画質・音質」）。「一般」タブに保存フォルダ・Cookies・言語選択を配置。Cookiesは「使用しない / ファイルを指定 / ブラウザから取得」の `QRadioButton` グループで切り替え、選択に応じてファイルパス入力欄（`QLineEdit` + `QFileDialog`）またはブラウザ選択コンボ（`QComboBox`）を `setVisible()` で排他表示する。保存時は非選択側のフィールドを空文字にリセット。言語変更時は再起動なしで即座に反映（`App._retranslate_ui()` を呼び出す）。「画質・音質」タブに解像度上限コンボ（480p〜2160p）・音声形式コンボ（MP3 / FLAC、`AUDIO_FORMATS` 定数と連動）・MP3ビットレートコンボ（128〜320kbps、MP3選択時のみ表示）を配置。

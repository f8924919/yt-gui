...
4.5. **設計レビュー（`design-review` 起動・条件付き）**（step 4.5）
   - **発火可否は主観で決めない**。investigate（step 3）の「設計レビュー推奨」と §5.5 のトリガで判定する。推奨が `yes`（トリガ該当）なら、docs に固めた設計案を `design-review` サブエージェント（Opus）で点検し、指摘・改善案を受け取る。
   ...
   - 推奨が `no`（トリガ非該当）なら省略してよい。`criteria-review`（step 3.5・条件の妥当性）とは対象が異なり、設計案そのものを見る。

...
- **タスクメモの形**: 冒頭に引用ブロック（Issue・ステータス・ブランチ・基点）、`## 進捗` は受け入れ条件ごとに `- [ ] Cn … — 証跡: 未`（段階を項目にしない）、`## 訂正ログ`（空でも見出しと表ヘッダ）、`## 次にやること`（末尾に `訂正ログ: N 件`）。**書いた主張が誤りだったと分かったら、その訂正コミットで `## 訂正ログ` に 1 行足す**。**2 件になったら止まって方針を問う**（git-workflow §5.2「訂正ログの止め規則」。docs-guide §3.2）。
